...
{
    public class BlogContentDal:DataOperationActivityBase<BlogContent>,IBlogContentDal
    {
        public virtual int DeleteByBlogFromUrl(string  url)
        {
            ...
            try
            {
                ...
                {
                    session.Delete(string.Format("from {0} where BlogFromUrl = '{1}'", typeof(BlogContent).Name, url));
                    session.Flush();
                }
            }
            catch (Exception ex)
